...
public class AdminChatDetailView {
    private BorderPane view = new BorderPane();
    private String studentName;

    public AdminChatDetailView(String studentName) {
        ...
        mainChat.setPadding(new Insets(0)); // Chat should be edge-to-edge in its container
        ...
        backBtn.setOnMouseClicked(e -> App.showAdminMessaging());
        
        VBox tBox = new VBox(2);
        Label n = new Label(studentName.toUpperCase());
        n.setStyle("-fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 18px; -fx-letter-spacing: 1px;");
        Label status = new Label("\u25cf MESSAGE LINK ACTIVE");
        ...
        inputField.setPromptText("Transmit encrypted message...");
        ...
        Runnable sendMessage = () -> {
            ...
            if (!text.isEmpty()) {
                App.Message msg = new App.Message(App.currentAdmin.name, studentName, text, "Just now", true);
                App.addMessage(studentName, msg);
                inputField.clear();
                renderMessages(messageList);
    ...
    private void renderMessages(VBox chatArea) {
        ...
        String key = studentName + ":" + App.currentAdmin.name;
    ...
    private HBox createMessageBubble(App.Message msg, VBox chatArea) {
        HBox row = new HBox();
        boolean isMine = msg.sender.equals(App.currentAdmin.name);
        ...
        deleteItem.setOnAction(e -> {
            App.deleteMessage(studentName, msg.id);
